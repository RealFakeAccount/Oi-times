...
namespace LinkCutTree {
	...
	void _splay(Node *&o) {
		...
		if (o->c != -1) {
			...
			if (p->c != -1) {
				_splay(p->ch[p->c]);
				if (o->c == p->c) rotate(o, o->c ^ 1); else rotate(o->ch[o->c], o->c);
			}
...
int main() {
	...
	for (int i = 0; i < m; ++i) {
		int op = readInt(), u = readInt() - 1, v = readInt() - 1;
		if (op == 1) LinkCutTree::link(u, v);
		else if (op == 2) {
			LinkCutTree::cut(u, v);
		} else if (op == 3) {
			if (!LinkCutTree::connected(u, v)) puts("naive");
			else printf("%d\n", LinkCutTree::sum(u, v));
		}
